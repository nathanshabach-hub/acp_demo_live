	...
	public function submitstudentevent($conv_reg_student_slug=NULL,$event_slug=NULL) {
		...
		if($event_slug)
		{
			...
			if(!empty($eventD->book_ids))
			{
				...
				$booksDD = $this->Books->find()->where($condEventBooks)->all()->combine('id', 'book_name')->toArray();
	...
	public function timeseventsentries($conv_reg_slug=null,$event_slug=null) {
		...
		if ($this->request->is(['post']))
		{
			...
			for($cntrE=1;$cntrE<=$total_records;$cntrE++)
			{
				...
				if(this->request->getData('withdrawn_'.$cntrE) !== null)
				{
					$withdraw_yes_no = 1;
				}
				else
	...
	public function scoreseventsentries($conv_reg_slug=null,$event_slug=null) {
		...
		if ($this->request->is(['post']))
		{
			...
			{	
				...
				{
					if(this->request->getData('pos_'.$cntPPS.'_'.$cntrE) !== null && !empty($this->request->getData('pos_'.$cntPPS.'_'.$cntrE)))
					{
						$arrPosYN[$cntPPS] 			= 1;
				...
				$pos_1						= $this->request->getData('pos_1_'.$cntrE);
				$pos_2						= $this->request->getData('pos_2_'.$cntrE);
				$pos_3						= $this->request->getData('pos_3_'.$cntrE);
				$pos_4						= $this->request->getData('pos_4_'.$cntrE);
				$pos_5						= $this->request->getData('pos_5_'.$cntrE);
				$pos_6						= $this->request->getData('pos_6_'.$cntrE);
				$pos_7						= $this->request->getData('pos_7_'.$cntrE);
				$pos_8						= $this->request->getData('pos_8_'.$cntrE);
				$pos_9						= $this->request->getData('pos_9_'.$cntrE);
				
				$comp_place_1				= $this->request->getData('comp_place_1_'.$cntrE);
				$comp_place_2				= $this->request->getData('comp_place_2_'.$cntrE);
				$comp_place_3				= $this->request->getData('comp_place_3_'.$cntrE);
