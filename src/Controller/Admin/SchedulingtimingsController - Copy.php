...
class SchedulingtimingsController extends AppController {
				...
				if(count((array)$mainArrForEvent))
				{
					...
					for($cntrEVSCH=0;$cntrEVSCH<count((array)$mainArrForEvent);$cntrEVSCH++)
					{
						...
						*/
						...
						$checkRoomAvailability = $this->Schedulingtimings->find()->where($condRAvail)->order(["Schedulingtimings.modified" => "DESC","Schedulingtimings.id" => "DESC"])->first();
	...
	public function listconflicts($convention_season_slug=null)
	{
		...
		foreach($schedulingtimings as $schrecord)
		{
			...
			// Group members - not considering now
		...
		$this->redirect(['controller' => 'schedulings', 'action' => 'listconflictsgroups', $convention_season_slug]);
	}
