...
class PaginatorHelper extends Helper
{
     ...
     * @var array
     */
    protected array $helpers = ['Url', 'Number', 'Html'];
    ...
    /**
     * Local cache of paging data (to allow mutation without modifying immutable request).
     *
     * @var array|null
     */
    ...
    /**
     * Get the paging data, preferring the local cache over the request param.
     *
     * @return array|null
     */
    protected function _getPaging()
    {
        ...
        return $this->getView()->getRequest()->getParam('paging');
    }
    ...
    public function __construct(View $View, array $config = [])
    {
        ...
        $this->setConfig(
            'options.url',
            array_merge($request->getAttribute('params')['pass'] ?? [], ['?' => $query])
        );
    }
    ...
    public function params($model = null)
    {
        ...
        if (empty($paging) || empty($paging[$model])) {
            return [];
        }
        return $paging[$model] + [
            'page' => 1,
    ...
    public function options(array $options = [])
    {
        ...
        if (!empty($options['paging'])) {
            ...
            $paging = $options['paging'] + $paging;
            unset($options['paging']);
        }
        $model = $this->defaultModel();

        if (!empty($options[$model])) {
            if (empty($paging[$model])) {
                $paging[$model] = [];
            }
            $paging[$model] = $options[$model] + $paging[$model];
            unset($options[$model]);
        }
        $this->_paging = $paging;
    ...
    public function defaultModel()
    {
        ...
        if (empty($this->_getPaging())) {
            return null;
        }
        list($this->_defaultModel) = array_keys($this->_getPaging());
        return $this->_defaultModel;
    }
    ...
    public function counter($options = [])
    {
        ...
        $paging = $this->params($options['model']);
        $paging += ['pageCount' => 0, 'count' => 0, 'perPage' => 1, 'page' => 1, 'current' => 0];
        if (!$paging['pageCount']) {
            $paging['pageCount'] = 1;
        }
        $start = 0;
    ...
    public function numbers(array $options = [])
    {
        ...
        $params = (array)$this->params($options['model']) + ['page' => 1, 'pageCount' => 0, 'current' => 0, 'count' => 0, 'perPage' => 1];
